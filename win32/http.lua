...
static const int HTTP_DEMAND_CBT                = 0x00000004;
]]
...
ffi.cdef[[
static const int HTTP_MAX_SERVER_QUEUE_LENGTH   = 0x7FFFFFFF;
static const int HTTP_MIN_SERVER_QUEUE_LENGTH   = 1;
]]
...
static const int HTTP_MIN_ALLOWED_BANDWIDTH_THROTTLING_RATE = ((ULONG)1024);
...
static const int HTTP_LIMIT_INFINITE  = ((ULONG)-1);
]]
...
// These timers run when theres a request being processed on a connection
...
static const int HTTP_AUTH_ENABLE_BASIC        =  (0x00000001);
static const int HTTP_AUTH_ENABLE_DIGEST       =  (0x00000002);
static const int HTTP_AUTH_ENABLE_NTLM         =  (0x00000004);
static const int HTTP_AUTH_ENABLE_NEGOTIATE    =  (0x00000008);
static const int HTTP_AUTH_ENABLE_KERBEROS     =  (0x00000010);

static const int HTTP_AUTH_ENABLE_ALL             \
...
ffi.cdef[[
static const int HTTP_AUTH_EX_FLAG_ENABLE_KERBEROS_CREDENTIAL_CACHING  =(0x01);
static const int HTTP_AUTH_EX_FLAG_CAPTURE_CREDENTIAL                  =(0x02);
]]
...
//
// HTTP API doesnt support 16 bit applications.
...
typedef struct _HTTP_RESPONSE_HEADERS
{
    ...
    //
    // Trailers - we dont use these currently, reserved for a future release
    //
    USHORT               TrailerCount;   // Reserved, must be 0
    PHTTP_UNKNOWN_HEADER pTrailers;      // Reserved, must be NULL
...
// if application has not asked for response to be cached. Its used as a hint
...
// AddrList[i].ss_family. If its AF_INET, use ((PSOCKADDR_IN) &AddrList[i]);
...
// config ID HttpServiceConfigUrlAclInfo. Its used to query a particular
...
HttpAddUrlToUrlGroup(
     ...
     HTTP_URL_CONTEXT UrlContext OPTIONAL,
      ULONG Reserved
    );
...
HttpSendHttpResponse(
     ...
     PHTTP_CACHE_POLICY CachePolicy OPTIONAL,
     PULONG BytesSent OPTIONAL,
     PVOID Reserved1,
     ULONG Reserved2,
     LPOVERLAPPED Overlapped OPTIONAL,
     PHTTP_LOG_DATA LogData OPTIONAL
    );
...
HttpSendHttpResponse(
     ...
     PVOID pReserved1 OPTIONAL, // must be NULL
     PULONG pBytesSent OPTIONAL,
     PVOID pReserved2, // must be NULL
     ULONG Reserved3, // must be 0
     LPOVERLAPPED pOverlapped OPTIONAL,
     PVOID pReserved4 OPTIONAL  // must be NULL
    );
...
HttpSendResponseEntityBody(
     ...
     HTTP_REQUEST_ID RequestId,
     ULONG Flags,
     USHORT EntityChunkCount OPTIONAL,
...
HttpSendResponseEntityBody(
    ...
    _In_reads_opt_(EntityChunkCount)
     PHTTP_DATA_CHUNK pEntityChunks OPTIONAL,
     PULONG pBytesSent OPTIONAL,
     PVOID pReserved1 OPTIONAL, // must be NULL
     ULONG Reserved2 OPTIONAL, // must be 0
     LPOVERLAPPED pOverlapped OPTIONAL,
     PVOID pReserved3 OPTIONAL  // must be NULL
    );
...
HttpReadFragmentFromCache(
     HANDLE RequestQueueHandle,
     PCWSTR UrlPrefix,
     PHTTP_BYTE_RANGE ByteRange OPTIONAL,
