...
local fourcc = require("win32.fourcc")
local MAKEFOURCC = fourcc.MAKEFOURCC


if not __DDRAW_INCLUDED__ then
__DDRAW_INCLUDED__ = true

require ("win32.winapifamily")


if WINAPI_FAMILY_PARTITION(__stdcall_PARTITION_DESKTOP) then
 ...
 */
 --]]
if not DIRECTDRAW_VERSION then
DIRECTDRAW_VERSION = 0x0700;
end --/* DIRECTDRAW_VERSION */


if _WIN32  and not _NO_COM then
COM_NO_WINDOWS_H = true
--require("win32.objbase")
else
--#define IUnknown            void
if not NT_BUILD_ENVIRONMENT and not WINNT then
ffi.cdef[[
...
local _FACDD  = 0x876
local function MAKE_DDHRESULT( code )  return MAKE_HRESULT( 1, _FACDD, code ) end
...
makeStatic("FOURCC_DXT1",  MAKEFOURCC('D','X','T','1'))
makeStatic("FOURCC_DXT2",  MAKEFOURCC('D','X','T','2'))
makeStatic("FOURCC_DXT3",  MAKEFOURCC('D','X','T','3'))
makeStatic("FOURCC_DXT4",  MAKEFOURCC('D','X','T','4'))
makeStatic("FOURCC_DXT5",  MAKEFOURCC('D','X','T','5'))
...
if (WIN32 or _WIN32 ) and not _NO_COM then
...
ffi.cdef[[
    ...
    extern _Check_return_ HRESULT __stdcall DirectDrawEnumerateW( LPDDENUMCALLBACKW lpCallback, LPVOID lpContext );
    extern _Check_return_ HRESULT __stdcall DirectDrawEnumerateA( LPDDENUMCALLBACKA lpCallback, LPVOID lpContext );
]]
    
-- * Protect against old SDKs
if not HMONITOR_DECLARED and (WINVER < 0x0500) then
...
ffi.cdef[[
    ...
    HRESULT __stdcall DirectDrawEnumerateExW( LPDDENUMCALLBACKEXW lpCallback, LPVOID lpContext, DWORD dwFlags);
 ...
 * DDCAPS
 */
static const int DD_ROP_SPACE           = (256/32);        // space required to store ROP array
/*
...
if DIRECTDRAW_VERSION <= 0x300 then
ffi.cdef[[
    typedef DDCAPS_DX3 DDCAPS;
]]
elseif DIRECTDRAW_VERSION <= 0x500 then
ffi.cdef[[
    typedef DDCAPS_DX5 DDCAPS;
]]
elseif DIRECTDRAW_VERSION <= 0x600 then
ffi.cdef[[
...
if not _DDPIXELFORMAT_DEFINED then
_DDPIXELFORMAT_DEFINED = true
...
static const int MAX_DDDEVICEID_STRING      =     512;
...
#ifdef STREAMING
typedef DWORD   ( __stdcall *LPSURFACESTREAMINGCALLBACK)(DWORD);
#endif
...
if not __cplusplus or defined(CINTERFACE) then
...
if DIRECTDRAW_VERSION < 0x0600 then
end  --DIRECTDRAW_VERSION
 ...
 * Generic failure.
 */
 // BUGBUG - has two possible definitions in winerror.lua
//static const int DDERR_GENERIC                        =   E_FAIL;
]]
 ...
 */
static const int DDERR_NOTINITIALIZED                 =   CO_E_NOTINITIALIZED;
]]


end --/* WINAPI_FAMILY_PARTITION(__stdcall_PARTITION_DESKTOP) */


end --__DDRAW_INCLUDED__
