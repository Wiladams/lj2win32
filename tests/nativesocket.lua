...
local ws2_32 = require("win32.winsock2");
...
function NativeSocket.init(self, sock, autoclose)
...
function NativeSocket.create(self, family, socktype, protocol, autoclose)
	family = family or ffi.C.AF_INET;
...
NativeSocket.createClient = function(self, hostname, port)
	local family = C.AF_INET;
	local socktype = C.SOCK_STREAM;
...
function NativeSocket.createServer(self, params)
	autoclose = autoclose or false;
...
function NativeSocket.getNativeSocket(self)
	return self.SafeHandle.sock;
end

function NativeSocket.getNativeHandle(self)
...
--]]
function NativeSocket.setAutoClose(self, autoclose)
	autoclose = autoclose or false;
...
-- basis.
function NativeSocket.setKeepAlive(self, keepalive, timeout, interval)
...
function NativeSocket.setNoDelay(self, nodelay)
...
function NativeSocket.setNonBlocking(self, nonblocking)
	local oneint = ffi.new("int[1]",0);
	if nonblocking then
		oneint[0] = 1
	end
...
function NativeSocket.setReuseAddress(self, reuse)
	local oneint = ffi.new("int[1]");
	if reuse then
		oneint[0] = 1
	end
...
function NativeSocket.setExclusiveAddress(self, exclusive)
	local oneint = ffi.new("int[1]");
	if exclusive then
		oneint[0] = 1
	end
...
--]]
function NativeSocket.getConnectionTime(self)
...
--]]
function NativeSocket.isConnected(self)
...
function NativeSocket.closeDown(self)
...
function NativeSocket.forceClose(self)
	return WinSock.closesocket(self:getNativeSocket());
end
		
function NativeSocket.shutdown(self, how)
	how = how or SD_SEND
...
--]]
function NativeSocket.createOverlapped(self, buff, bufflen, operation)
...
--]]
function NativeSocket.connectTo(self, address)
...
--]]
function NativeSocket.makePassive(self, backlog)
...
function NativeSocket.bind(self, addr, addrlen)
	return WinSock.bind(self:getNativeSocket(), addr, addrlen)
end

function NativeSocket.bindToPort(self, port)
...
function NativeSocket.send(self, buff, bufflen)
...
function NativeSocket.sendTo(self, lpTo, iTolen, buff, bufflen)
	if lpTo == nil then
		return false;
	end
...
function NativeSocket.receive(self, buff, bufflen)
...
function NativeSocket.receiveFrom(self, lpFrom, fromLen, buff, bufflen)
